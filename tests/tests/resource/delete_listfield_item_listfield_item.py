...
class ResourceDeleteListFieldItemListFieldItem(unittest.TestCase):
    """
    Test if a HTTP DELETE request on a item in a listfield in an item of
    a listfield deletes the item and gives the right response.
    """
    ...
    def setUpClass(cls):
        ...
        cls.response = cls.app.delete(
            '/articles/{}/comments/{}/upvotes/{}/'.format(
                article_id, comment_id, ip_address
            )
        )
    ...
    def test_deleted(self):
        ...
        """
        self.assertEqual(len(Article.objects[0].comments[0].upvotes), 1)
